...
void draw(){
  ...
  if(start==true){
    //resetting the background every 1 to get rid of motion blur
    if(c==1){
    ...
    textAlign(LEFT);
    fill(0);
    text("Points: "+(score + score_reduction),10,10);
    fill(0);
    text("Round: "+wave,(width/2)-20,10);
    for(int i=0;i<particle_dead.length;i++)
      if(particle_dead[i]==false)
        particle_count++;
    text("Enemies: "+ particle_count,width-100,10);
    crosshair();
    ...
    for(int i=0;i<p.length;i++){
      ...
      //drawing cities and Magazines every frame for each particle detect collision with each
      add_cities(position.x,position.y);
      draw_Magazines(position.x,position.y);
    }
...
//add the Magazines each round and reset the missile counter sso that each Magazine has 10 missile
void add_Magazines(){
  dead_missiles=0;
  float dimensions[]= new float[4];
  for(int i=0;i<s.length;i++){
    Magazine_dead[i]=false;
    switch(i){
      case 0: {
    ...
    s[i]=new Magazine(dimensions[0],dimensions[1],dimensions[0]+dimensions[2],dimensions[1]+dimensions[3]);
  }
}

//drawing the Magazines each frame with the updated missile counter aand their state(alive or dead)
void draw_Magazines(float px,float py){
  for(int i=0;i<s.length;i++){
    if(s[i].isAlive(px,py) && Magazine_dead[i]==false){
      noStroke();
      fill(58,74,44);
...
//handling the mouse click event to store the coordinates of the click to launch a missile to the destination from the nearest Magazine with usaable missiles
void mouseClicked(){
...
//handling key press events to give the user control over while Magazine they want the missile to be launched from
void keyPressed(){
...
//the function actually creating the missile object and giving it initial velocity (which is slightly higher if shot from the center missile Magazine)
